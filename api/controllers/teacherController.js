...
exports.getShortTeachersList = function (request, response) {
    model.Teacher.findAll({
            raw: true
        })
        .then(teachers => {
            const resObj = teachers.map(teacher => {
                return Object.assign({}, {
                    teacher_id: teacher.id,
                    shortName: `${teacher.lastName} ${teacher.firstName.charAt(0)}.${teacher.middleName.charAt(0)}.`
                });
            })
        ...
        .then(user => {
            ...
            if (isValid) {
                ...
                if (user.lastName == 'Admin') {
                    ...
                    name =  `${user.lastName} ${user.firstName.charAt(0)}.${user.middleName.charAt(0)}.`
                }
                response.json({
                    token: token,
                    name: name,
